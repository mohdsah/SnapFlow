...
-- ✅ Uncomment dan run baris ini dalam Supabase SQL Editor
-- ATAU pergi ke Storage → New Bucket dalam dashboard

insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values ('videos', 'videos', true, 104857600, array['video/mp4','video/webm','video/quicktime','video/avi','video/mov'])
on conflict (id) do nothing;

insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values ('media', 'media', true, 10485760, array['image/jpeg','image/png','image/gif','image/webp'])
on conflict (id) do nothing;
...
create policy "media bucket: auth user upload"
    on storage.objects for insert
    with check (
        bucket_id = 'media'
        and auth.role() = 'authenticated'
